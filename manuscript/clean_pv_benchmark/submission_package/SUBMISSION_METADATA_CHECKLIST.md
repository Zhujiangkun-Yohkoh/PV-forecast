# JRSE submission metadata checklist

Official requirements checked on **2026-08-28** against the JRSE scope and charges pages, AIP author instructions, and the [AIP AI policy](https://publishing.aip.org/resources/researchers/policies-and-ethics/ai-policy/).

## Confirmed manuscript metadata

- Target: *Journal of Renewable and Sustainable Energy*; Research Article.
- Title: *Leakage-Aware Multi-Horizon Benchmarking of Compact Neural PV Forecasts Across Co-Located Technologies*.
- Status: `JRSE_SCIENTIFIC_REVISION_READY_FOR_AUTHOR_APPROVAL`; declarations are not final.
- Author order: Jiangkun Zhu; Mengling Yang; Zhicong Chen; Lijun Wu.
- Affiliation 1: College of Advanced Manufacturing, Fuzhou University, 1 Shuicheng Road, Quanzhou 362251, China.
- Affiliation 2: College of Physics and Information Engineering, Fuzhou University, 2 Xueyuan Road, Fuzhou 350116, China.
- Jiangkun Zhu: affiliation 1; ORCID 0009-0009-5335-2345.
- Mengling Yang: affiliation 1; ORCID optional and not provided.
- Zhicong Chen: affiliations 1 and 2; corresponding author; `zhicong.chen@fzu.edu.cn`; ORCID optional and not provided.
- Lijun Wu: affiliations 1 and 2; corresponding author; `lijun.wu@fzu.edu.cn`; ORCID optional and not provided.
- Exclusive submission: **confirmed by user (YES)**.
- Suggested reviewers: **None provided**.
- Opposed reviewers: **None**.
- Publishing route: `TRADITIONAL_ROUTE_PLANNED`; do not select optional Author Select OA or authorize the USD 3,800 fee without a new author decision.
- Indexing: `INDEXING_STATUS_REQUIRES_AUTHOR_OR_LIBRARY_CONFIRMATION`.

## Final author confirmation required

- [ ] Final title and complete revised manuscript approved by all four authors.
- [ ] Both corresponding-author designations and emails confirmed.
- [ ] Optional ORCIDs for Mengling Yang, Zhicong Chen, and Lijun Wu supplied or intentionally left blank.
- [ ] CRediT draft confirmed by every author.
- [ ] Funding draft and Grant Nos. 62271151 and W2421092 confirmed as applicable to this Scheme A paper.
- [ ] Conflict of Interest draft confirmed.
- [ ] AI-assisted-tools disclosure completed with exact tool name, version, provider, task, and rationale as required by AIP policy.
- [ ] Data Availability and Code Availability wording confirmed against the eventual public release.
- [ ] Ethics submission-system answer confirmed by the authors.
- [ ] Exclusive-submission answer re-confirmed at upload.
- [ ] All authors approve submission and the final cover letter.
- [ ] Related manuscript/preprint disclosure completed.
- [ ] Suggested reviewers supplied only if the live system requires them.
- [ ] Traditional/subscription route selected; fee responsibility confirmed if any optional service is chosen.

## Files and accessibility

- [ ] Obtain all-author approval for the scientifically revised main and supplementary PDFs before upload.
- [ ] Confirm every figure and table is cited, numbered, captioned, and legible.
- [x] Prepare English alt text for all main and supplementary figures and principal tables.
- [ ] At source-file request, upload `main.tex`, `supplementary.tex`, `references.bib`, generated table sources, and five vector figure PDFs.
- [ ] Confirm PDF fonts are embedded and declarations contain no draft marker.

## Official-information boundary

- JRSE's official page states no page charges and lists Author Select as optional; the planned route is traditional/subscription.
- AIP permits AI-assisted manuscript preparation, but use that may affect findings, analysis, or information synthesis must be disclosed with exact tool details; AI cannot be an author.
- Current SCIE/JCR status remains for author or institutional-library confirmation; it is not guessed here.

