...
\hypersetup{colorlinks=true,linkcolor=blue!45!black,urlcolor=blue!45!black,pdftitle={Supplementary Material: Leakage-Aware Multi-Horizon Benchmarking of Compact Neural PV Forecasts}}
\begin{document}
...
\author{Zhicong Chen}
\author{Lijun Wu}
\date{August 28, 2026 --- WORKING MATERIAL FOR AUTHOR APPROVAL}
\maketitle

\section{Evidence scope}
This supplement reports only the frozen evidence used by the main manuscript. No model was trained or tuned during manuscript revision. The benchmark comprises four compact neural implementations, three co-located photovoltaic arrays, three random seeds (42, 43, and 44), and direct 144-step forecasts at five-minute resolution. Primary H12, H48, H96, and H144 metrics use horizon-specific valid origins. Daily Persistence comparisons instead use an elementwise intersection mask that is valid for the daily lag, labels, Last-value Persistence, and every neural forecast.
...
An earlier diagnostic demonstrated why a baseline's missing lag points cannot be deleted from that baseline alone. Such a comparison changes the target set and can reverse a headline count even when every numerical metric is computed correctly. The final Daily-matched analysis instead applies one point mask to Daily Persistence, Last-value Persistence, all neural seeds, and the labels. No unmatched-sample number is used as a model-performance result.
