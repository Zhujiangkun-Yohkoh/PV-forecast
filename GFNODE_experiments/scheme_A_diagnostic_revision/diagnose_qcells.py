...
def run(paths):
 c=json.loads(Path(paths).read_text(encoding='utf8'));cfg=json.loads((HERE.parent/'scheme_A_submission_correction/config.json').read_text(encoding='utf8'));raw=pd.read_csv(c['alice_raw']['Qcells']);raw.timestamp=pd.to_datetime(raw.timestamp);raw=raw.sort_values('timestamp').drop_duplicates('timestamp',keep='last').set_index('timestamp');grid=pd.date_range(raw.index.min(),raw.index.max(),freq='5min');power=pd.to_numeric(raw.Active_Power,errors='coerce').reindex(grid);threshold=.01*power.loc[slice(*cfg['splits']['train'])].where(lambda v:v>=0).max();tables=[];hours=[];months=[];reasons=[];flags=[];hist=[];checks=[]
 ...
 pd.concat(flags).to_csv(out/'qcells_origin_flags.csv',index=False);(out/'QCELLS_ALIGNMENT.json').write_text(json.dumps(dict(threshold=threshold,checks=checks),indent=2), encoding='utf8');print(pd.DataFrame(tables).query("split=='test' and h==12").to_string(index=False));print(pd.DataFrame(reasons).query("split=='test' and population=='H12_support'").to_string(index=False))
