...
def run(paths,destination,sites,matrix_cache=None):
 ...
 for site in sites:
  ...
  for variant in ['Ridge','Ridge+day']:
   ...
   stem=variant.replace('+','_');np.savez_compressed(folder/(stem+'_predictions.npz'),predictions=p,**{k:v for k,v in a.items() if k!='predictions'});np.savez_compressed(folder/(stem+'_coefficients.npz'),**(c|{'coef':coef}));(folder/(stem+'_completed.json')).write_text(json.dumps(record,indent=2))
   if site=='YULARA_COMBINED':
