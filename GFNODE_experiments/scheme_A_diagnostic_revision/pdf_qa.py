...
def run(dest):
 ...
 for name,build in [('main','tex-diag-main'),('supplementary','tex-diag-supp')]:
  ...
  fonts=subprocess.run(['pdffonts',str(pdf)],capture_output=True,text=True).stdout;log=pdf.with_suffix('.log').read_text(errors='replace');results[name]=dict(pages=pages,font_table=fonts,overfull_lines=[x for x in log.splitlines() if 'Overfull' in x],undefined_references=bool(re.search(r'(Reference .*undefined|Citation .*undefined|multiply defined)',log)))
 ...
 (HERE/'results/PDF_QA.json').write_text(json.dumps(results,indent=2));print(json.dumps({k:{'pages':len(v['pages']),'minimum_figure_font':min([p['figure_min_font_pt'] for p in v['pages'] if p['figure_min_font_pt'] is not None]),'overfull':v['overfull_lines']} for k,v in results.items()},indent=2))
