...
def run(paths,expanded,destination):
 ...
 def add(mapping,p,rel,role):
  p=Path(p);assert p.is_file(),p;assert rel not in mapping,rel;assert not any(x in p.parts for x in ['__pycache__','.git']);assert (p.suffix.lower() not in ['.zip','.pyc'] or p.name=='official_2018_ground.zip') and p.name!='.env';s=p.stat();stats[str(p)]=(s.st_size,s.st_mtime_ns);mapping[rel]=(p,role)
 prefixes=['GFNODE_experiments/scheme_A_submission_correction/','GFNODE_experiments/scheme_A_multisite_extension/','GFNODE_experiments/scheme_A_review_extension/','GFNODE_experiments/scheme_A_diagnostic_revision/','GFNODE_experiments/scheme_A_fixed_period_review/','GFNODE_experiments/scheme_A_support_closeout/','manuscript/clean_pv_benchmark/']
 ...
 for p in n.rglob('*'):
  ...
  # Explicit fixed-period evidence; original official download ZIP is source data,
 # not an old review archive. No directory-wide ZIP inclusion is permitted.
 for key,label in [('destination','new_frozen_review'),('nist_2018','nist_2018'),('closeout_destination','support_closeout')]:
  src=Path(cfg[key])
  for p in src.rglob('*'):
   if p.is_file() and p.suffix not in ['.pyc'] and (p.suffix!='.zip' or p.name=='official_2018_ground.zip'):
    add(full,p,('data/' if label=='nist_2018' else 'evidence/')+label+'/'+p.relative_to(src).as_posix(),'source / saved frozen-model forwards; Alice acceptance is per-method in current closeout status')
 assert sum(k.endswith('best_validation.pt') for k in full)==60
 assert sum(k.startswith('data/') and k.endswith('.csv') for k in full)==463
 ...
 configs.update({'expanded_results':{s:'../evidence/expanded_ridge/'+s for s in cfg['expanded_results']},'destination':'../evidence/new_frozen_review','closeout_destination':'../evidence/support_closeout','nist_2018':'../data/nist_2018'})
 ...
 figure={k:v for k,v in project.items() if '/closeout_figures/' in k or k.endswith('/scheme_A_fixed_period_review/build_figures.py') or k.endswith('/scheme_A_support_closeout/build_figures.py') or k.endswith('/scheme_A_diagnostic_revision/additional_figures.py') or k.endswith('/scheme_A_diagnostic_revision/build_figures.py') or ('/scheme_A_review_extension/results/' in k and k.endswith('.csv')) or ('/scheme_A_diagnostic_revision/results/' in k and k.endswith('.csv')) or k.endswith('/scheme_A_submission_correction/corrected_metrics.csv') or ('/scheme_A_fixed_period_review/results/' in k and k.endswith('.csv')) or ('/scheme_A_support_closeout/results/' in k and k.endswith('.csv')) or k.endswith('/FIGURE_QA.md')}
 outcomes=[]
...
Closeout scientific state B: a narrower verified core can enter manuscript preparation. The original Alice tolerance record remains 16 passed / 20 outside tolerance, but metric impacts are quantified and accepted methods now have S1/S2 comparisons. Hanwha/Qcells have full Inverted three-seed results; Sanyo has only its verified single seed. Diagnostic methods are separate, never used to fabricate a mean. Read project/PROJECT_REPORT_CN.md and project/GFNODE_experiments/scheme_A_support_closeout/REPORT_CN.md, and the current PDFs. PR_DESCRIPTION_DRAFT.md is local only, not posted. Plotting entry: python GFNODE_experiments/scheme_A_support_closeout/build_figures.py. The figure handoff contains the earlier plotting sources needed for retained figures as well.
...
''').encode()
  if kind=='Full_Project_Review':
   extra['project/closeout_paths.json']=json.dumps(configs,indent=2).encode();extra['project/review_paths.json']=json.dumps(configs,indent=2).encode();extra['project/external_paths.json']=json.dumps({'YULARA_RAW_FILE':'../data/Yulara/'+y.name,'NIST_GROUND_2017_DIRECTORY':'../data/NIST'},indent=2).encode();extra['project/expanded_paths.json']=json.dumps({'expanded_results':'../evidence/expanded_ridge'},indent=2).encode()
...
Full and Lite run the current verify_light.py, independently replaying 6120 paired rows from block-count weights without the production interval function and reporting dynamic per-method Alice replay status. This is not checkpoint replay or full neural training. Figure package checks all 22 PDF/SVG/PNG/source/caption/alt groups; the complete figures were generated and visually inspected in the original revision before packaging. No training runs occur during extraction checks.
''').encode()
  ...
  if kind!='Figure_Handoff':
   process=subprocess.run([sys.executable,'-X','utf8','-B','GFNODE_experiments/scheme_A_support_closeout/verify_light.py'],cwd=extract/'project',capture_output=True,text=True,encoding='utf8');assert process.returncode==0,process.stdout+process.stderr;check=process.stdout
  else:
   figs=extract/'project/manuscript/clean_pv_benchmark/closeout_figures';names=[p.stem for p in figs.glob('fig*.pdf')];assert len(names)==22
   for stem in names:
    for ending in ['.pdf','.svg','.png','_data.csv','_caption.txt','_alt.txt']:assert (figs/(stem+ending)).stat().st_size>0
   check='22 figure groups / six required assets per group; no heavy prediction replay.'
