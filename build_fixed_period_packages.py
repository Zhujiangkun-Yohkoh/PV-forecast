...
def run(paths,expanded,destination):
 ...
 prefixes=['GFNODE_experiments/scheme_A_submission_correction/','GFNODE_experiments/scheme_A_multisite_extension/','GFNODE_experiments/scheme_A_review_extension/','GFNODE_experiments/scheme_A_diagnostic_revision/','GFNODE_experiments/scheme_A_fixed_period_review/','manuscript/clean_pv_benchmark/']
 ...
 for key,label in [('destination','new_frozen_review'),('nist_2018','nist_2018')]:
  src=Path(cfg[key])
  for p in src.rglob('*'):
   if p.is_file() and p.suffix not in ['.pyc'] and (p.suffix!='.zip' or p.name=='official_2018_ground.zip'):
    add(full,p,('data/' if label=='nist_2018' else 'evidence/')+label+'/'+p.relative_to(src).as_posix(),'fixed-period source / new frozen-prediction evidence; Alice replay unaccepted')
...
A/C are complete; B is PARTIAL: 20/36 Alice neural replays do not meet the historical tolerance. New Alice arrays are unaccepted diagnostics, not accepted S1/S2 scores. Read project/PROJECT_REPORT_CN.md and project/GFNODE_experiments/scheme_A_fixed_period_review/REPORT_CN.md, and the latest paper under project/manuscript/clean_pv_benchmark. The figure-only package contains its complete plotting inputs and QA instead of the manuscript and heavy evidence. Run plotting from project with python GFNODE_experiments/scheme_A_fixed_period_review/build_figures.py (numpy/pandas/matplotlib).
...
Full and Lite run the current verify_light.py, replaying 1368 paired rows from block SSE/counts and preserving the explicit 20/36 unresolved Alice neural replay status. This is not checkpoint replay or full neural training. Figure package checks all 21 PDF/SVG/PNG/source/caption/alt groups; the complete figures were generated and visually inspected in the original revision before packaging. No training runs occur during extraction checks.
''').encode()
  ...
  if kind!='Figure_Handoff':
   process=subprocess.run([sys.executable,'-X','utf8','-B','GFNODE_experiments/scheme_A_fixed_period_review/verify_light.py'],cwd=extract/'project',capture_output=True,text=True,encoding='utf8');assert process.returncode==0,process.stdout+process.stderr;check=process.stdout
  else:
   figs=extract/'project/manuscript/clean_pv_benchmark/fixed_period_figures';names=[p.stem for p in figs.glob('fig*.pdf')];assert len(names)==21
   for stem in names:
    for ending in ['.pdf','.svg','.png','_data.csv','_caption.txt','_alt.txt']:assert (figs/(stem+ending)).stat().st_size>0
   check='21 figure groups / six required assets per group; no heavy prediction replay.'
